...
public class BPlusTreeDeadlockTest extends SimpleDbTestBase {
	private Random rand;
	...
	@Before public void setUp() throws Exception {
		...
		bf = BPlusTreeUtility.createRandomBPlusTreeFile(2, 253008, null, null, 0);
		rand = new Random();
		item1 = rand.nextInt(BPlusTreeUtility.MAX_RAND_VALUE);
		item2 = rand.nextInt(BPlusTreeUtility.MAX_RAND_VALUE);
	...
	/**
	 * Helper method to clean up the syntax of starting a BPlusTreeWriter thread.
	 * The parameters pass through to the BPlusTreeWriter constructor.
	 */
	public BPlusTreeUtility.BPlusTreeWriter startWriter(TransactionId tid, 
			int item, int count) {

		BPlusTreeWriter bw = new BPlusTreeWriter(tid, bf, item, count);
		bw.start();
		return bw;
	}
	...
	@Test public void testReadWriteDeadlock() throws Exception {
		...
		Database.getBufferPool().getPage(tid1, BPlusTreeRootPtrPage.getId(bf.getId()), Permissions.READ_ONLY);
		Database.getBufferPool().getPage(tid2, BPlusTreeRootPtrPage.getId(bf.getId()), Permissions.READ_ONLY);

		// allow read locks to acquire
		Thread.sleep(POLL_INTERVAL);
		
		BPlusTreeWriter writer1 = startWriter(tid1, item1, count1);
		BPlusTreeWriter writer2 = startWriter(tid2, item2, count2);
	...
	public static junit.framework.Test suite() {
		return new JUnit4TestAdapter(BPlusTreeDeadlockTest.class);
	}
