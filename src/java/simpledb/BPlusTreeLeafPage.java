 ...
 *
 */
public class BPlusTreeLeafPage implements Page {
	 ...
	 * The format of a BPlusTreeLeafPage is a set of header bytes indicating
	 ...
	 */
	public BPlusTreeLeafPage(BPlusTreePageId id, byte[] data, int key) throws IOException {
		this.pid = id;
		this.keyField = key;
	 ...
	 * @return the number of bytes in the header of a page in a BPlusTreeFile with each tuple occupying tupleSize bytes
	 */
	...
	/** Return a view of this page before it was modified
        -- used by recovery */
	public BPlusTreeLeafPage getBeforeImage(){
		try {
			...
			return new BPlusTreeLeafPage(pid,oldDataRef,keyField);
		} catch (IOException e) {
			e.printStackTrace();
	 ...
	 * array generated by getPageData to the BPlusTreeLeafPage constructor and
	 * have it produce an identical BPlusTreeLeafPage object.
	 *
	 ...
	 * this method to the BPlusTreeLeafPage constructor will create a BPlusTreeLeafPage with
	...
	public void insertTuple(Tuple t) throws DbException {
		...
		Debug.log(1, "BPlusTreeLeafPage.insertTuple: new tuple, tableId = %d pageId = %d slotId = %d", pid.getTableId(), pid.pageNumber(), goodSlot);
	 ...
	 * @return the id of the parent of this page
	 */
	 ...
	 */
	public BPlusTreePageId getLeftSiblingId() {
		if(leftSibling == 0) {
			return null;
		}
		return new BPlusTreePageId(pid.getTableId(), leftSibling, BPlusTreePageId.LEAF);
	}
	 ...
	 */
	public BPlusTreePageId getRightSiblingId() {
		if(rightSibling == 0) {
			return null;
		}
		return new BPlusTreePageId(pid.getTableId(), rightSibling, BPlusTreePageId.LEAF);
	}
	 ...
	 * @throws DbException if the id is not valid
	 */
	public void setLeftSiblingId(BPlusTreePageId id) throws DbException {
		if(id == null) {
			leftSibling = 0;
		}
		else {
			...
			if(id.pgcateg() != BPlusTreePageId.LEAF) {
	 ...
	 * @throws DbException if the id is not valid
	 */
	public void setRightSiblingId(BPlusTreePageId id) throws DbException {
		if(id == null) {
			rightSibling = 0;
		}
		else {
	...
	private void markSlotUsed(int i, boolean value) {
		...
		Debug.log(1, "BPlusTreeLeafPage.setSlot: setting slot %d to %b", i, value);
		if(value)
	...
	public Iterator<Tuple> iterator() {
		return new BPlusTreeLeafPageIterator(this);
	}
	...
	Tuple getTuple(int i) throws NoSuchElementException {
		...
		try {
			if(!isSlotUsed(i)) {
				Debug.log(1, "BPlusTreeLeafPage.getTuple: slot %d in %d:%d is not used", i, pid.getTableId(), pid.pageNumber());
				return null;
			}

			Debug.log(1, "BPlusTreeLeafPage.getTuple: returning tuple %d", i);
			return tuples[i];
 ...
 */
class BPlusTreeLeafPageIterator implements Iterator<Tuple> {
	int curTuple = 0;
	Tuple nextToReturn = null;
	BPlusTreeLeafPage p;

	public BPlusTreeLeafPageIterator(BPlusTreeLeafPage p) {
		this.p = p;
	}
