...
public class BPlusTreeFileEncoder {
	 ...
	 * @param bFile - the file on disk to back the resulting BPlusTreeFile
	 * @param keyField - the index of the key field for this B+ tree
	 * @param numFields - the number of fields in each tuple
	 * @return the BPlusTreeFile
	 */
	public static BPlusTreeFile convert(ArrayList<ArrayList<Integer>> tuples, File hFile, 
	 ...
	 * @return the BPlusTreeFile
	 */
	public static BPlusTreeFile convert(File inFile, File hFile, File bFile,
		...
		BPlusTreeFile bf = BPlusTreeUtility.openBPlusTreeFile(numFields, bFile, keyField);
	 ...
	 * @param bFile - the data file for the BPlusTreeFile
	 * @param npagebytes - number of bytes per page
	 * @param numFields - number of fields per tuple
	 * @param typeAr - array containing the types of the tuples
	 * @param fieldSeparator - character separating fields in the raw data file
	 ...
	 */
	public static BPlusTreeFile convert(File inFile, File hFile, File bFile, int npagebytes,
			int numFields, Type[] typeAr, char fieldSeparator, int keyField) 
					throws IOException, DbException, TransactionAbortedException {
		// convert the inFile to HeapFile first.
		HeapFileEncoder.convert(inFile, hFile, BufferPool.getPageSize(), numFields);
		...
		int leafpointerbytes = 3 * BPlusTreeLeafPage.INDEX_SIZE; 
		int nrecords = (npagebytes * 8 - leafpointerbytes * 8) /  (nrecbytes * 8 + 1);  //floor comes for free

		int nentrybytes = keyType.getLen() + BPlusTreeInternalPage.INDEX_SIZE;
		// pointerbytes: one extra child pointer, parent pointer, child page category
		int internalpointerbytes = 2 * BPlusTreeLeafPage.INDEX_SIZE + 1; 
		int nentries = (npagebytes * 8 - internalpointerbytes * 8 - 1) /  (nentrybytes * 8 + 1);  //floor comes for free

		ArrayList<ArrayList<BPlusTreeEntry>> entries = new ArrayList<ArrayList<BPlusTreeEntry>>();

		// first add some bytes for the root pointer page
		bf.writePage(new BPlusTreeRootPtrPage(BPlusTreeRootPtrPage.getId(tableid), 
				BPlusTreeRootPtrPage.createEmptyPageData()));
		...
		for(Tuple tup : tuples) {
			if(page1.size() < nrecords) {
				...
				BPlusTreePageId leafPid = new BPlusTreePageId(tableid, bf.numPages() + 1, BPlusTreePageId.LEAF);
				BPlusTreeLeafPage leafPage = new BPlusTreeLeafPage(leafPid, leafPageBytes, keyField);
				...
				BPlusTreeEntry copyUpEntry = new BPlusTreeEntry(page2.get(0).getField(keyField), leafPid, null);
		...
		if(page2.size() == 0) {
			...
			lastPid = new BPlusTreePageId(tableid, bf.numPages() + 1, BPlusTreePageId.LEAF);
			BPlusTreeLeafPage lastPage = new BPlusTreeLeafPage(lastPid, lastPageBytes, keyField);
			...
			BPlusTreePageId secondToLastPid = new BPlusTreePageId(tableid, bf.numPages() + 1, BPlusTreePageId.LEAF);
			BPlusTreeLeafPage secondToLastPage = new BPlusTreeLeafPage(secondToLastPid, secondToLastPageBytes, keyField);
			...
			// update the parent by "copying up" the next key
			BPlusTreeEntry copyUpEntry = new BPlusTreeEntry(lastPg.get(0).getField(keyField), secondToLastPid, lastPid);
			updateEntries(entries, bf, copyUpEntry, 0, nentries, npagebytes, 
					keyType, tableid, keyField);
		}
		...
		int rootCategory = (root > 1 ? BPlusTreePageId.INTERNAL : BPlusTreePageId.LEAF);
		byte[] rootPtrBytes = convertToRootPtrPage(root, rootCategory, 0);
		bf.writePage(new BPlusTreeRootPtrPage(BPlusTreeRootPtrPage.getId(tableid), rootPtrBytes));

		// set all the parent and sibling pointers
		setParents(bf, new BPlusTreePageId(tableid, root, rootCategory), BPlusTreeRootPtrPage.getId(tableid));
	 ...
	 * @throws DbException
	 */
	private static void setRightSiblingPtrs(BPlusTreeFile bf, BPlusTreePageId pid, BPlusTreePageId rightSiblingId) 
			throws IOException, DbException {
		BPlusTreeLeafPage page = (BPlusTreeLeafPage) bf.readPage(pid);
		page.setRightSiblingId(rightSiblingId);
		BPlusTreePageId leftSiblingId = page.getLeftSiblingId();
		bf.writePage(page);
	 ...
	 * @throws DbException
	 */
	private static void setParents(BPlusTreeFile bf, BPlusTreePageId pid, BPlusTreePageId parent) 
			throws IOException, DbException {
		if(pid.pgcateg() == BPlusTreePageId.INTERNAL) {
			BPlusTreeInternalPage page = (BPlusTreeInternalPage) bf.readPage(pid);
		...
		else { // pid.pgcateg() == BPlusTreePageId.LEAF
			BPlusTreeLeafPage page = (BPlusTreeLeafPage) bf.readPage(pid);
			page.setParentId(parent);
			bf.writePage(page);
		}
	 ...
	 * @param tableid - the table id of this BPlusTreeFile
	 * @param keyField - the index of the key field
	 * @throws IOException
	 */
	private static void cleanUpEntries(ArrayList<ArrayList<BPlusTreeEntry>> entries,
			BPlusTreeFile bf, int nentries, int npagebytes, Type keyType, int tableid, 
		...
		for(int i = 0; i < entries.size(); i++) {
			int childPageCategory = (i == 0 ? BPlusTreePageId.LEAF : BPlusTreePageId.INTERNAL);
			...
			if(size <= nentries) {
				...
				BPlusTreePageId internalPid = new BPlusTreePageId(tableid, bf.numPages() + 1, BPlusTreePageId.INTERNAL);
				bf.writePage(new BPlusTreeInternalPage(internalPid, internalPageBytes, keyField));
			}
			else {
				// split the remaining entries in half
				ArrayList<BPlusTreeEntry> secondToLastPg = new ArrayList<BPlusTreeEntry>();
				ArrayList<BPlusTreeEntry> lastPg = new ArrayList<BPlusTreeEntry>();
				...
				BPlusTreePageId secondToLastPid = new BPlusTreePageId(tableid, bf.numPages() + 1, BPlusTreePageId.INTERNAL);
				bf.writePage(new BPlusTreeInternalPage(secondToLastPid, secondToLastPageBytes, keyField));

				byte[] lastPageBytes = convertToInternalPage(lastPg, npagebytes, keyType, childPageCategory);
				BPlusTreePageId lastPid = new BPlusTreePageId(tableid, bf.numPages() + 1, BPlusTreePageId.INTERNAL);
				bf.writePage(new BPlusTreeInternalPage(lastPid, lastPageBytes, keyField));

				// update the parent by "pushing up" the next key
				BPlusTreeEntry pushUpEntry = new BPlusTreeEntry(entries.get(i).get(size/2).getKey(), secondToLastPid, lastPid);
	 ...
	 * @throws IOException
	 */
	private static void updateEntries(ArrayList<ArrayList<BPlusTreeEntry>> entries, 
			BPlusTreeFile bf, BPlusTreeEntry e, int level, int nentries, int npagebytes, Type keyType, 
			int tableid, int keyField) throws IOException {
		while(entries.size() <= level) {
			entries.add(new ArrayList<BPlusTreeEntry>());
		}

		int childPageCategory = (level == 0 ? BPlusTreePageId.LEAF : BPlusTreePageId.INTERNAL);
		int size = entries.get(level).size();

		if(size > 0) {
			BPlusTreeEntry prev = entries.get(level).get(size-1);
			entries.get(level).set(size-1, new BPlusTreeEntry(prev.getKey(), prev.getLeftChild(), e.getLeftChild()));
			if(size == nentries * 2 + 1) {
				// write out a page of entries
				ArrayList<BPlusTreeEntry> pageEntries = new ArrayList<BPlusTreeEntry>();
				...
				// update the parent by "pushing up" the next key
				BPlusTreeEntry pushUpEntry = new BPlusTreeEntry(entries.get(level).get(nentries).getKey(), internalPid, null);
				updateEntries(entries, bf, pushUpEntry, level + 1, nentries, npagebytes, 
						keyType, tableid, keyField);
				ArrayList<BPlusTreeEntry> remainingEntries = new ArrayList<BPlusTreeEntry>();
	 ...
	 * @param typeAr - array containing the types of the tuples
	 * @param keyField - the field of the tuples the B+ tree will be keyed on
	 * @return a byte array which can be passed to the BPlusTreeLeafPage constructor
	 * @throws IOException
	 */
	public static byte[] convertToLeafPage(ArrayList<Tuple> tuples, int npagebytes,
		...
		// pointerbytes: left sibling pointer, right sibling pointer, parent pointer
		int pointerbytes = 3 * BPlusTreeLeafPage.INDEX_SIZE; 
	 ...
	 */
	public static class EntryComparator implements Comparator<BPlusTreeEntry> {
		/**
		 * Compare two entries based on their key field
		 * 
		 * @return -1 if e1 < e2, 1 if e1 > e2, 0 if e1 == e2
		 */
		public int compare(BPlusTreeEntry e1, BPlusTreeEntry e2) {
			int cmp = 0;
	 ...
	 * @return a byte array which can be passed to the BPlusTreeInternalPage constructor
	 * @throws IOException
	 */
	public static byte[] convertToInternalPage(ArrayList<BPlusTreeEntry> entries, int npagebytes,
			Type keyType, int childPageCategory)
					throws IOException {
		int nentrybytes = keyType.getLen() + BPlusTreeInternalPage.INDEX_SIZE;
		// pointerbytes: one extra child pointer, parent pointer, child page category
		int pointerbytes = 2 * BPlusTreeLeafPage.INDEX_SIZE + 1; 
		...
		for(int e = entrycount; e < nentries; e++) {
			for (int j=0; j<BPlusTreeInternalPage.INDEX_SIZE; j++) {
				dos.writeByte(0);
			}
		}
	 ...
	 * @return a byte array which can be passed to the BPlusTreeRootPtrPage constructor
	 * @throws IOException
	 */
	public static byte[] convertToRootPtrPage(int root, int rootCategory, int header)
			throws IOException {

		ByteArrayOutputStream baos = new ByteArrayOutputStream(BPlusTreeRootPtrPage.getPageSize());
		DataOutputStream dos = new DataOutputStream(baos);
