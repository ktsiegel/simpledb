 ...
 * @see BufferPool
 */
public class BPlusTreeRootPtrPage implements Page {
	// size of this page
	...
	/**
	 * Constructor.
	 * Construct the BPlusTreeRootPtrPage from a set of bytes of data read from
	 * disk.
	 * The format of an BPlusTreeRootPtrPage is an integer for the page number
	 ...
	 */
	public BPlusTreeRootPtrPage(BPlusTreePageId id, byte[] data) throws IOException {
		this.pid = id;
	...
	/**
	 * There is only one instance of a BPlusTreeRootPtrPage per table. This static 
	 ...
	 */
	public static BPlusTreePageId getId(int tableid) {
		return new BPlusTreePageId(tableid, 0, BPlusTreePageId.ROOT_PTR);
	}
	 ...
	 * generated by getPageData to the BPlusTreeRootPtrPage constructor and have it
	 * produce an identical BPlusTreeRootPtrPage object.
	 *
	 ...
	 * this method to the BPlusTreeRootPtrPage constructor will create a BPlusTreeRootPtrPage with
	...
	/** Return a view of this page before it was modified
        -- used by recovery */
	public BPlusTreeRootPtrPage getBeforeImage(){
		try {
			return new BPlusTreeRootPtrPage(pid,oldData);
		} catch (IOException e) {
			e.printStackTrace();
	 ...
	 */
	public BPlusTreePageId getRootId() {
		if(root == 0) {
			return null;
		}
		return new BPlusTreePageId(pid.getTableId(), root, rootCategory);
	}
	 ...
	 */
	public void setRootId(BPlusTreePageId id) throws DbException {
		if(id == null) {
			root = 0;
		}
		else {
			...
			if(id.pgcateg() != BPlusTreePageId.INTERNAL && id.pgcateg() != BPlusTreePageId.LEAF) {
	 ...
	 */
	public BPlusTreePageId getHeaderId() {
		if(header == 0) {
			return null;
		}
		return new BPlusTreePageId(pid.getTableId(), header, BPlusTreePageId.HEADER);
	}
	 ...
	 * @throws DbException if the id is invalid
	 */
	public void setHeaderId(BPlusTreePageId id) throws DbException {
		if(id == null) {
			header = 0;
		}
		else {
			...
			if(id.pgcateg() != BPlusTreePageId.HEADER) {
				throw new DbException("header must be of type BPlusTreePageId.HEADER");
			}
